...
func TestPhase0B5ControlAcceptsEquivalentKeyOrderAndUnicodeRepresentation(t *testing.T) {
	...
	if _, err := Validate(safePolicy().ControlToken, []byte(reordered), safePolicy()); err != nil {
		t.Fatalf("semantically equivalent key order and Unicode escape were rejected: %v", err)
	}
}
