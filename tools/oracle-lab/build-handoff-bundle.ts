...
export function validateHandoffValue(value: unknown, now = Date.now(), verifyArtifacts = true): HarnessResult {
  ...
  else for (const [index, repository] of value.repositories.entries()) { if (!exactKeys(repository, ['name', 'commit', 'dirty_digest'], `$.repositories[${index}]`, errors)) continue; if (typeof repository.name !== 'string' || !/^[0-9a-f]{40,64}$/.test(String(repository.commit)) || !/^sha256:[0-9a-f]{64}$/.test(String(repository.dirty_digest))) errors.push({ code: 'missing_repository_digests', path: `$.repositories[${index}]`, message: 'invalid repository provenance' }) }
  if (!Array.isArray(value.commands) || value.commands.some((command) => !isObject(command) || typeof command.command_id !== 'string' || typeof command.status !== 'string' || !/^sha256:[0-9a-f]{64}$/.test(String(command.result_digest)))) errors.push({ code: 'invalid_commands', path: '$.commands', message: 'invalid command summaries' })
  ...
  if (!isObject(value.retention_policy) || value.retention_policy.digest_only !== 'phase_evidence_permanent' || value.retention_policy.redacted_excerpt !== '7_days') errors.push({ code: 'invalid_retention_policy', path: '$.retention_policy', message: 'invalid retention policy' })
  if (typeof value.redaction_policy !== 'string' || typeof value.destruction_procedure !== 'string') errors.push({ code: 'invalid_metadata', path: '$', message: 'redaction and destruction policy are required' })
  return result(errors)
}

export function buildHandoff(options: { phase: string; baseline: string; commandResults: string; context?: string; out?: string; generatedAt?: string }): HandoffBundle {
  ...
  if (options.context) { const value = readJson(options.context); requireValid(validateContextPackValue(value)); context = value as ContextPack }
...
if (realpathSync(process.argv[1] ?? '') === fileURLToPath(import.meta.url)) cli(() => {
  ...
  writeJson(out, buildHandoff({ phase, baseline, commandResults, context: args.values.context?.[0] }))
})
