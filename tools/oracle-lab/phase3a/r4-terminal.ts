...
function tierASupportDigest(index: Record<string, any>, r2Sha256: string, r3Sha256: string, rerunSha256: string): string {
  ...
  const support = [...new Map([...projections, ...bindings, r2, r3, rerun].map((row) => [String(row.artifact_id), { artifact_id: row.artifact_id, sha256: row.sha256, relative_path: row.relative_path }])).values()]
...
export function buildR4TerminalManifest(input: Input): Record<string, any> {
  ...
  const tierASupportSha256 = tierASupportDigest(input.index, input.r2_sha256, input.r3_sha256, input.tier_a_rerun_sha256)
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  expectedEvidenceInput(root, values.leak!, 'guards/leak-scan-v19.json', 'leak scan')
