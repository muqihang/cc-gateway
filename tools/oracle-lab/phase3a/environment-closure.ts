...
export function closeEnvironmentMatrix(input: { matrix_sha256: string; expected: ExpectedRow[]; rows: ClosureRow[] }): Record<string, any> {
  ...
  for (const row of input.rows) {
    ...
    if (!['REPRODUCED', 'UNKNOWN'].includes(row.status) || !/^[a-f0-9]{64}$/.test(row.source_sha256)) fail('matrix_closure_incomplete', `pair has an invalid status or digest binding: ${row.pair_id}`)
  ...
  const statuses = pairs.reduce<Record<string, number>>((counts, row) => { counts[row.status] = (counts[row.status] ?? 0) + 1; return counts }, {})
  const base = { schema_version: 'oracle-lab-phase3a-environment-matrix-closure.v1', status: statuses.UNKNOWN ? 'CLOSED_WITH_UNKNOWN' : 'PASS', matrix_sha256: input.matrix_sha256, pair_count: pairs.length, families, effects, statuses, pairs, external_socket_budget: 0, raw_material_persisted: false }
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  const coreRows = new Map(core.pairs.map((row) => [row.pair_id, row]))
  const rows: ClosureRow[] = matrix.pairs.map((pair, index) => {
    const coreRow = coreRows.get(pair.pair_id)
    if (pair.family !== 'provider-token' && coreRow) return { pair_id: pair.pair_id, status: coreRow.status, effect: coreRow.effect, source: 'core-reclassification', source_sha256: sha256File(coreFile) }
