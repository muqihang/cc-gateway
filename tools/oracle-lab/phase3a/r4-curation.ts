...
export function closureConclusions(r2Sha256: string, r3Sha256: string): any[] {
  const reproduced = (id: string, statement: string, support: string[], runs: string[], controls: string[]) => ({
    conclusion: {
      ...
      dynamic_reproduction: { run_ids: runs, control_run_ids: controls, source_count: 3 }, single_source_reason: null,
  ...
  const unknown = (id: string, statement: string, negative: string) => ({
    conclusion: {
      ...
      scope: 'claude-code-2.1.215 darwin-arm64 synthetic loopback fixtures', statement,
      supporting_artifact_ids: ['p3a2-closure-coverage'], contradicting_artifact_ids: [], static_anchor: null, dynamic_reproduction: null,
  ...
  return [
    reproduced('CL-P3A-R2-ROUTING-CONFIG-AUTH', 'Routing, config precedence, and placeholder credential lifecycle were stable in the bounded local campaign.', ['p3a2-closure-coverage', `r2-${r2Sha256}`], ['closure-r2-config-v2', 'closure-r2-auth-v1', 'closure-r2-auth-co-v2'], ['closure-r2-config-v2-control']),
    reproduced('CL-P3A-R2-FAILURE-STREAM', 'HTTP failure, reset, partial stream, and complete stream terminal classes were stable in the bounded local campaign.', ['p3a2-closure-scenarios', `r3-${r3Sha256}`], ['closure-r2-scenario-v2', 'closure-r2-partial-v6', 'closure-r2-complete-v7'], ['closure-r2-scenario-v2-control']),
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const required = ['template', 'r2', 'r3', 'artifact-index', 'leak-scan', 'cc-root', 'sub2api-root', 'cc-base', 'sub2api-base', 'cc-freeze', 'sub2api-freeze', 'out-input', 'out-exit', 'out-markdown', 'out-handoff']
  ...
  const input: CuratedExitInput = {
    ...
    conclusions: closureConclusions(r2Sha, r3Sha),
  ...
  for (const file of [values['out-input']!, values['out-exit']!, values['out-markdown']!, values['out-handoff']!]) mkdirSync(path.dirname(file), { recursive: true, mode: 0o700 })
