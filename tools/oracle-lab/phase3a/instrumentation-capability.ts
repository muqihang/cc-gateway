...
function pairedManifest(base: LaunchManifest, runId: string, sequenceIndex: number, hook: boolean): LaunchManifest {
  return {
    ...structuredClone(base),
    run_id: runId,
    pair_id: 'closure-r2-bun-instrumentation-capability-v1',
    sequence_index: sequenceIndex,
    randomization_seed: 8215,
    hypothesis_id: 'active-2.1.215-bun-preload-reachability-and-perturbation',
...
export async function runInstrumentationCapability(options: CapabilityOptions): Promise<Record<string, unknown>> {
  ...
  try {
    const base = buildBaselineManifest({
      ...
      run_id: 'closure-r2-capability-base', cc_commit: options.cc_commit, cc_tree: options.cc_tree,
    ...
    const control = pairedManifest(base, 'closure-r2-capability-bun-control-v1', 0, false)
    const treatment = pairedManifest(base, 'closure-r2-capability-bun-treatment-v1', 1, true)
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    ...
    const required = ['evidence-root', 'entrypoint', 'out-relative', 'cc-commit', 'cc-tree', 'sub2api-commit', 'sub2api-tree', 'plan-sha256', 'toolchain-digest', 'static-inventory-sha256']
    ...
    const result = await runInstrumentationCapability({
      evidence_root: values['evidence-root'], entrypoint: path.resolve(values.entrypoint), out_relative: values['out-relative'],
