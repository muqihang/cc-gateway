...
export function validateCommandCatalogValue(value: unknown, requirementIds?: ReadonlySet<string>): HarnessResult {
  ...
  for (const [index, item] of value.entries()) {
    ...
    if (exactKeys(item.manifest_binding, bindingFields.filter((key) => key !== 'contract_digest_field' || isObject(item.manifest_binding) && key in item.manifest_binding), `${base}.manifest_binding`, errors)) {
      if (typeof item.manifest_binding.manifest_path !== 'string' || !expansions(item.manifest_binding.manifest_path).every((name) => name === 'ENTRY_MANIFEST' || name === 'EXIT_MANIFEST')) errors.push({ code: 'invalid_manifest_binding', path: `${base}.manifest_binding.manifest_path`, message: 'manifest_path must use a manifest placeholder' })
      if (typeof item.manifest_binding.repository_head_field !== 'string' || !/^repositories\.(cc_gateway|sub2api)\.head$/.test(item.manifest_binding.repository_head_field)) errors.push({ code: 'invalid_manifest_binding', path: `${base}.manifest_binding.repository_head_field`, message: 'invalid head field' })
      if ('contract_digest_field' in item.manifest_binding && item.manifest_binding.contract_digest_field !== 'contract.sha256') errors.push({ code: 'invalid_manifest_binding', path: `${base}.manifest_binding.contract_digest_field`, message: 'invalid contract field' })
    }
