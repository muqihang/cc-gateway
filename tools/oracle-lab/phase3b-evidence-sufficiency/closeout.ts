...
const OBSERVATION_FIELDS = ['schema_id', 'campaign_id', 'ledger_sha256', 'run_id', 'sequence_index', 'receiver_group_id', 'receiver_instance_id', 'receiver_authority_sha256', 'target_pid', 'target_instance_id', 'executable_identity_sha256', 'route_ordinal', 'connection_ordinal', 'raw_socket_ordinal', 'attempt_ordinal', 'action_ordinal', 'peer_socket', 'method', 'path', 'query_present', 'query_order', 'query_items', 'ordered_header_classes', 'header_presence', 'auth_marker_winner_class', 'body_byte_length', 'body_sha256', 'body_ast', 'body_ast_sha256', 'body_normalized_byte_length', 'body_normalized_sha256', 'body_roundtrip_sha256', 'response_program_sha256', 'response', 'observation_sha256'] as const
...
function validateObservation(value: Record<string, unknown>, row: RunLedgerRow): Readonly<{ sha256: string; projection: Readonly<Record<string, unknown>> }> {
  ...
  if ((row.family === 'auth' || row.family === 'config') && value.auth_marker_winner_class !== expectedAuthMarkerClass(row)) throw new Phase3BProductionError('observation_invalid', 'actual synthetic auth marker does not match the sealed arm')
  ...
  return deepFreeze({
    ...
    projection: projectValidatedObservationForControlStability({
      ...
      query_order: value.query_order,
      query_items: value.query_items,
...
export function validateReceiverOrdinalBindings(receiver: Readonly<Record<string, unknown>>, observations: readonly Readonly<Record<string, unknown>>[], row: RunLedgerRow): void {
  ...
  const rawSockets = receiver.raw_socket_ordinals
  const actions = receiver.action_ordinals
  const digests = receiver.observation_sha256s
  ...
  const bootstrapDigestValid = !!bootstrap && bootstrap.bootstrap_sha256 === sha256Canonical(Object.fromEntries(Object.entries(bootstrap).filter(([key]) => key !== 'bootstrap_sha256')))
  const bootstrapValid = bootstrapDigestValid && (expectedBootstrap === 1
    ? !!bootstrap && !!bootstrapPeer && bootstrapPeer.peer_socket_sha256 === sha256Canonical(Object.fromEntries(Object.entries(bootstrapPeer).filter(([key]) => key !== 'peer_socket_sha256'))) && bootstrap.count === 1 && bootstrap.route_ordinal === row.bootstrap_contract.expected_route_ordinal && bootstrap.raw_socket_ordinal === 0 && bootstrap.response_status === 200 && bootstrap.response_content_length === 0 && bootstrap.response_finished === true && bootstrap.socket_closed === true && bootstrap.socket_close_had_error === false && bootstrap.post_count_effect === 0
    : !!bootstrap && bootstrap.count === 0 && bootstrap.route_ordinal === null && bootstrap.receiver_instance_id === null && bootstrap.raw_socket_ordinal === null && bootstrap.peer_socket === null && bootstrap.response_status === null && bootstrap.response_content_length === null && bootstrap.response_finished === null && bootstrap.socket_closed === null && bootstrap.socket_close_had_error === null && bootstrap.post_count_effect === 0)
  if (sha256Canonical(receiver.bootstrap_contract) !== sha256Canonical(row.bootstrap_contract) || receiver.selected_route_ordinal !== expectedSelectedRoute(row) || receiver.receiver_terminal !== expectedTerminal || receiver.request_count !== maximumAttempts || receiver.response_count !== maximumAttempts || !exactVector(attempts) || !exactVector(connections) || !exactVector(rawSockets, expectedBootstrap) || !Array.isArray(actions) || actions.length !== maximumAttempts || !Array.isArray(digests) || digests.length !== maximumAttempts || observations.length !== maximumAttempts || actions.some((value, index) => value !== row.response_program.actions[index].action_ordinal) || !bootstrapValid) throw new Phase3BProductionError('receiver_terminal_invalid', 'receiver result cardinality, route contract, bootstrap, terminal, or ordinal vector drifted')
  const requestPeer = observations[0]?.peer_socket as Record<string, unknown> | undefined
  for (let index = 0; index < maximumAttempts; index += 1) {
    ...
    const targetPeer = expectedBootstrap === 1 ? bootstrapPeer : requestPeer
    if (observation.attempt_ordinal !== index || observation.connection_ordinal !== index || observation.raw_socket_ordinal !== index + expectedBootstrap || observation.action_ordinal !== actions[index] || observation.observation_sha256 !== sha256Canonical(Object.fromEntries(Object.entries(observation).filter(([key]) => key !== 'observation_sha256'))) || observation.observation_sha256 !== digests[index] || !observationPeer || !requestPeer || !targetPeer || observationPeer.peer_socket_sha256 !== sha256Canonical(Object.fromEntries(Object.entries(observationPeer).filter(([key]) => key !== 'peer_socket_sha256'))) || observationPeer.target_pid !== targetPeer.target_pid || observationPeer.executable_identity_sha256 !== targetPeer.executable_identity_sha256 || observationPeer.local_address !== '127.0.0.1' || observationPeer.remote_address !== '127.0.0.1' || observationPeer.local_port !== requestPeer.local_port || sha256Canonical(observation.query_order) !== sha256Canonical(row.request_target.query_order) || sha256Canonical(observation.query_items) !== sha256Canonical(row.request_target.query_items)) throw new Phase3BProductionError('receiver_terminal_invalid', 'receiver result does not cross-bind the exact sealed observation order, peer, and digest')
  }
}

export function validateObservationRouteAuthorityBindings(observations: readonly Readonly<Record<string, unknown>>[], row: RunLedgerRow, selectedRoute: Readonly<Record<string, unknown>>, executableIdentitySha256: string): void {
  if (selectedRoute.route_ordinal !== expectedSelectedRoute(row) || selectedRoute.host !== '127.0.0.1' || !Number.isSafeInteger(selectedRoute.port)) throw new Phase3BProductionError('receiver_authority_invalid', 'selected request route authority drifted')
  for (const observation of observations) {
    const peer = observation.peer_socket as Record<string, unknown> | undefined
    if (observation.route_ordinal !== expectedSelectedRoute(row) || observation.receiver_instance_id !== selectedRoute.receiver_instance_id || !peer || peer.local_address !== '127.0.0.1' || peer.remote_address !== '127.0.0.1' || peer.local_port !== selectedRoute.port || peer.executable_identity_sha256 !== executableIdentitySha256) throw new Phase3BProductionError('receiver_authority_invalid', 'POST observation is not bound to the selected request route')
  }
}
...
export function validateReceiverAuthorityClosureBindings(input: Readonly<{ ledger: CampaignLedger; row: RunLedgerRow; receiver: Readonly<Record<string, unknown>>; receiver_authority: Readonly<Record<string, unknown>>; launch_authority: Readonly<Record<string, unknown>>; static_anchor: Readonly<Record<string, unknown>>; prelaunch: Readonly<Record<string, unknown>>; operator_authority: Readonly<Record<string, unknown>>; campaign_input: Readonly<Record<string, unknown>> }>): void {
  const { ledger, row, receiver, receiver_authority: authority, launch_authority: launch, static_anchor: anchor, prelaunch, operator_authority: operator, campaign_input: campaignInput } = input
  const selectedRoute = Array.isArray(authority.routes) ? authority.routes[expectedSelectedRoute(row)] as Record<string, unknown> | undefined : undefined
  const expectedBootstrap = expectedBootstrapRoute(row)
  const bootstrapRoute = expectedBootstrap === null || !Array.isArray(authority.routes) ? undefined : authority.routes[expectedBootstrap] as Record<string, unknown> | undefined
  const bootstrap = receiver.bootstrap as Record<string, unknown> | undefined
  const bootstrapPeer = bootstrap?.peer_socket as Record<string, unknown> | undefined
  const bootstrapCount = expectedBootstrapCount(row)
  const bootstrapIdentityValid = bootstrapCount === 1
    ? bootstrap?.count === 1 && bootstrap.route_ordinal === expectedBootstrap && bootstrap.receiver_instance_id === bootstrapRoute?.receiver_instance_id && !!bootstrapPeer && bootstrapPeer.local_address === '127.0.0.1' && bootstrapPeer.remote_address === '127.0.0.1' && bootstrapPeer.local_port === bootstrapRoute?.port && bootstrapPeer.executable_identity_sha256 === launch.executable_identity_sha256
    : bootstrap?.count === 0 && bootstrap.route_ordinal === null && bootstrap.receiver_instance_id === null && bootstrapPeer === null
  const exactRoutePolicy = Array.isArray(authority.routes) && authority.routes.length === row.route_count && authority.routes.every((candidate, index) => {
    const route = candidate as Record<string, unknown>
    return route.route_ordinal === index && route.host === '127.0.0.1' && Number.isSafeInteger(route.port)
  })
  if (anchor.schema_id !== 'oracle-lab-p3b-static-anchor.v1' || operator.campaign_id !== ledger.campaign_id || campaignInput.campaign_id !== ledger.campaign_id || operator.campaign_input_sha256 !== campaignInput.input_sha256 || authority.schema_id !== 'oracle-lab-p3b-receiver-authority.v1' || authority.campaign_id !== ledger.campaign_id || authority.ledger_sha256 !== ledger.ledger_sha256 || authority.run_id !== row.run_id || authority.sequence_index !== row.sequence_index || authority.receiver_group_id !== row.receiver_group_id || authority.response_program_sha256 !== row.response_program_sha256 || authority.selected_route_ordinal !== expectedSelectedRoute(row) || sha256Canonical(authority.bootstrap_contract) !== sha256Canonical(row.bootstrap_contract) || authority.anchor_sha256 !== anchor.anchor_sha256 || authority.receiver_source_sha256 !== anchor.receiver_source_sha256 || authority.receiver_executable_identity_sha256 !== anchor.receiver_executable_identity_sha256 || authority.receiver_schema_sha256 !== anchor.receiver_schema_sha256 || sha256Canonical(anchor.request_target) !== sha256Canonical(ledger.request_target) || sha256Canonical(anchor.bootstrap_contract_schema) !== sha256Canonical(BOOTSTRAP_CONTRACT_SCHEMA) || prelaunch.schema_id !== 'oracle-lab-p3b-prelaunch-result.v1' || prelaunch.campaign_id !== ledger.campaign_id || prelaunch.authority_sha256 !== operator.authority_sha256 || prelaunch.input_sha256 !== campaignInput.input_sha256 || prelaunch.ledger_sha256 !== ledger.ledger_sha256 || prelaunch.anchor_sha256 !== anchor.anchor_sha256 || prelaunch.status !== 'SEALED' || !exactRoutePolicy || selectedRoute?.route_ordinal !== expectedSelectedRoute(row) || selectedRoute.host !== '127.0.0.1' || !bootstrapIdentityValid || launch.schema_id !== 'oracle-lab-p3b-launch-authority.v1' || launch.campaign_id !== ledger.campaign_id || launch.ledger_sha256 !== ledger.ledger_sha256 || launch.run_id !== row.run_id || launch.sequence_index !== row.sequence_index || launch.anchor_sha256 !== anchor.anchor_sha256 || launch.response_program_sha256 !== row.response_program_sha256 || launch.receiver_authority_sha256 !== authority.authority_sha256 || receiver.schema_id !== 'oracle-lab-p3b-receiver-result.v1' || receiver.campaign_id !== ledger.campaign_id || receiver.ledger_sha256 !== ledger.ledger_sha256 || receiver.run_id !== row.run_id || receiver.sequence_index !== row.sequence_index || receiver.receiver_group_id !== row.receiver_group_id || receiver.receiver_authority_sha256 !== authority.authority_sha256 || receiver.selected_route_ordinal !== expectedSelectedRoute(row) || sha256Canonical(receiver.bootstrap_contract) !== sha256Canonical(row.bootstrap_contract)) throw new Phase3BProductionError('receiver_authority_invalid', 'receiver authority/result/launch/static/prelaunch identity tuple drifted')
}

function classifyRow(root: string, ledger: CampaignLedger, row: RunLedgerRow, terminal: ExecutionReceipt | null): Readonly<Record<string, unknown>> {
  ...
  const staticAnchor = optionalCanonical(root, 'prelaunch/static-anchor.json')
  const prelaunch = optionalCanonical(root, 'prelaunch/result.json')
  const operatorAuthority = optionalCanonical(root, 'control/operator-authority.json')
  const campaignInput = optionalCanonical(root, 'control/campaign-input.json')
  if (!receiver || !receiverAuthority || !launchAuthority || !inputDescriptor || !cell || !guard || !staticAnchor || !prelaunch || !operatorAuthority || !campaignInput) return unknownRow(row, 'sealed_result_missing')
  assertExactKeys(receiver.value, ['schema_id', 'campaign_id', 'ledger_sha256', 'run_id', 'sequence_index', 'receiver_group_id', 'receiver_authority_sha256', 'selected_route_ordinal', 'bootstrap_contract', 'bootstrap', 'listener_retirement', 'request_count', 'response_count', 'route_request_counts', 'attempt_ordinals', 'connection_ordinals', 'raw_socket_ordinals', 'action_ordinals', 'observation_sha256s', 'receiver_terminal', 'result_sha256'], 'receiver_terminal_invalid')
  assertExactKeys(receiver.value.bootstrap, ['count', 'route_ordinal', 'receiver_instance_id', 'raw_socket_ordinal', 'peer_socket', 'response_status', 'response_content_length', 'response_finished', 'socket_closed', 'socket_close_had_error', 'post_count_effect', 'bootstrap_sha256'], 'receiver_terminal_invalid')
  if ((receiver.value.bootstrap as Record<string, unknown>).peer_socket !== null) assertExactKeys((receiver.value.bootstrap as Record<string, unknown>).peer_socket, ['target_pid', 'local_address', 'local_port', 'remote_address', 'remote_port', 'executable_identity_sha256', 'peer_socket_sha256'], 'receiver_terminal_invalid')
  const listenerRetirement = receiver.value.listener_retirement as Record<string, unknown> | null
  if (listenerRetirement !== null) assertExactKeys(listenerRetirement, ['route_ordinal', 'receiver_instance_id', 'port', 'prior_listener_identity_sha256', 'retired', 'listener_unowned', 'retired_monotonic_ns', 'retirement_sha256'], 'receiver_terminal_invalid')
  assertExactKeys(receiverAuthority.value, ['schema_id', 'campaign_id', 'ledger_sha256', 'run_id', 'sequence_index', 'receiver_group_id', 'receiver_pid', 'receiver_executable_identity_sha256', 'receiver_source_sha256', 'receiver_schema_sha256', 'anchor_sha256', 'response_program_sha256', 'selected_route_ordinal', 'bootstrap_contract', 'routes', 'authority_sha256'], 'receiver_authority_invalid')
  ...
  for (const route of receiverAuthority.value.routes) assertExactKeys(route, ['route_ordinal', 'receiver_instance_id', 'host', 'port', 'listener_identity_sha256'], 'receiver_authority_invalid')
  ...
  assertDigestField(receiver.value.bootstrap, 'bootstrap_sha256', 'receiver_terminal_invalid')
  ...
  assertDigestField(staticAnchor.value, 'anchor_sha256', 'static_anchor_invalid')
  assertExactKeys(prelaunch.value, ['schema_id', 'campaign_id', 'authority_sha256', 'input_sha256', 'ledger_sha256', 'anchor_sha256', 'launch_image_set_sha256', 'target_launches', 'receiver_binds', 'external_sockets', 'status', 'prelaunch_result_sha256'], 'prelaunch_invalid')
  assertDigestField(prelaunch.value, 'prelaunch_result_sha256', 'prelaunch_invalid')
  assertDigestField(operatorAuthority.value, 'authority_sha256', 'operator_authority_invalid')
  assertDigestField(campaignInput.value, 'input_sha256', 'campaign_input_invalid')
  ...
  const rawSockets = receiver.value.raw_socket_ordinals
  ...
  if (row.schedule_id === 'partial_sse_then_eof') {
    if (!listenerRetirement || listenerRetirement.route_ordinal !== expectedSelectedRoute(row) || listenerRetirement.receiver_instance_id !== selectedAuthorityRoute?.receiver_instance_id || listenerRetirement.port !== selectedAuthorityRoute?.port || listenerRetirement.prior_listener_identity_sha256 !== selectedAuthorityRoute?.listener_identity_sha256 || listenerRetirement.retired !== true || listenerRetirement.listener_unowned !== true || !/^\d+$/.test(String(listenerRetirement.retired_monotonic_ns)) || listenerRetirement.retirement_sha256 !== sha256Canonical(Object.fromEntries(Object.entries(listenerRetirement).filter(([key]) => key !== 'retirement_sha256')))) return unknownRow(row, 'receiver_listener_retirement_mismatch')
  } else if (listenerRetirement !== null) return unknownRow(row, 'receiver_listener_retirement_unexpected')
  ...
  for (let attempt = 0; attempt < expectedAttempts; attempt += 1) {
    ...
    sealedObservations.push(observation.value)
    observationProjections.push(validated.projection)
  }
  try { validateObservationRouteAuthorityBindings(sealedObservations, row, selectedAuthorityRoute ?? {}, String(launchAuthority.value.executable_identity_sha256)) } catch { return unknownRow(row, 'receiver_authority_binding_mismatch') }
...
export function sealTargetControlTranche(evidenceRoot: string): Readonly<Record<string, unknown>> {
  ...
  const rows = enforcePairAndRepetitionStability(controls, controls.map((row) => classifyRow(root, ledger, row, terminalByRow.get(row.sequence_index) ?? null)))
...
function deriveFixtureRows(root: string, ledger: CampaignLedger): readonly Readonly<Record<string, unknown>>[] {
  ...
  return deepFreeze(ledger.rows.map((row) => {
    ...
    const requests = observations.map(({ relativePath, value, identity }) => {
      const request = { schema_id: 'oracle-lab-p3b-typed-request-fixture.v1', method: value.method, path: value.path, query_present: value.query_present, query_order: value.query_order, query_items: value.query_items, ordered_header_classes: value.ordered_header_classes, header_presence: value.header_presence, auth_marker_winner_class: value.auth_marker_winner_class, body_byte_length: value.body_byte_length, body_sha256: value.body_sha256, body_ast: value.body_ast, body_ast_sha256: value.body_ast_sha256, body_normalized_byte_length: value.body_normalized_byte_length, body_normalized_sha256: value.body_normalized_sha256, body_roundtrip_sha256: value.body_roundtrip_sha256 }
...
function deriveCapturedSourceValue(descriptor: Record<string, unknown>, leaf: string, ledger: CampaignLedger, fixtureRows: readonly Readonly<Record<string, unknown>>[], targetProfile: TargetProfile): unknown {
  ...
  if (leaf === '/request/method') return ledger.request_target.method
  if (leaf === '/request/target/path') return ledger.request_target.path
...
export function deriveCuration(evidenceRoot: string, allowTestAttestation = false): Readonly<Record<string, unknown>> {
  ...
  const rows = enforcePairAndRepetitionStability(ledger.rows, ledger.rows.map((row) => classifyRow(root, ledger, row, terminalByRow.get(row.sequence_index) ?? null)))
