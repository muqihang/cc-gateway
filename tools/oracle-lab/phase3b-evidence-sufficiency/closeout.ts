...
export function validateObservationRouteAuthorityBindings(observations: readonly Readonly<Record<string, unknown>>[], row: RunLedgerRow, selectedRoute: Readonly<Record<string, unknown>>, executableIdentitySha256: string): void {
  if (selectedRoute.route_ordinal !== expectedSelectedRoute(row) || selectedRoute.expected_selected !== true || selectedRoute.host !== '127.0.0.1' || !Number.isSafeInteger(selectedRoute.port)) throw new Phase3BProductionError('receiver_authority_invalid', 'selected request route authority drifted')
...
export function validateReceiverAuthorityClosureBindings(input: Readonly<{ ledger: CampaignLedger; row: RunLedgerRow; receiver: Readonly<Record<string, unknown>>; receiver_authority: Readonly<Record<string, unknown>>; launch_authority: Readonly<Record<string, unknown>>; static_anchor: Readonly<Record<string, unknown>>; prelaunch: Readonly<Record<string, unknown>>; operator_authority: Readonly<Record<string, unknown>>; campaign_input: Readonly<Record<string, unknown>> }>): void {
  ...
  const bootstrapIdentityValid = bootstrapCount === 1
    ? bootstrap?.count === 1 && bootstrap.policy === 'exact_one_loopback_head' && bootstrap.route_ordinal === expectedBootstrap && bootstrap.receiver_instance_id === bootstrapRoute?.receiver_instance_id && bootstrapRoute?.expected_bootstrap === true && !!bootstrapPeer && bootstrapPeer.local_address === '127.0.0.1' && bootstrapPeer.remote_address === '127.0.0.1' && bootstrapPeer.local_port === bootstrapRoute?.port && bootstrapPeer.executable_identity_sha256 === launch.executable_identity_sha256
    : bootstrap?.count === 0 && bootstrap.policy === 'config_file_precedence_no_bootstrap' && bootstrap.route_ordinal === null && bootstrap.receiver_instance_id === null && bootstrapPeer === null
...
function classifyRow(root: string, ledger: CampaignLedger, row: RunLedgerRow, terminal: ExecutionReceipt | null): Readonly<Record<string, unknown>> {
  ...
  assertExactKeys(receiver.value, ['schema_id', 'campaign_id', 'ledger_sha256', 'run_id', 'sequence_index', 'receiver_group_id', 'receiver_authority_sha256', 'bootstrap', 'request_count', 'response_count', 'route_request_counts', 'attempt_ordinals', 'connection_ordinals', 'raw_socket_ordinals', 'action_ordinals', 'observation_sha256s', 'receiver_terminal', 'result_sha256'], 'receiver_terminal_invalid')
  assertExactKeys(receiver.value.bootstrap, ['count', 'policy', 'route_ordinal', 'receiver_instance_id', 'raw_socket_ordinal', 'peer_socket', 'response_status', 'response_content_length', 'response_finished', 'socket_closed', 'socket_close_had_error', 'post_count_effect', 'bootstrap_sha256'], 'receiver_terminal_invalid')
  ...
  for (const route of receiverAuthority.value.routes) assertExactKeys(route, ['route_ordinal', 'receiver_instance_id', 'host', 'port', 'expected_selected', 'expected_bootstrap', 'listener_identity_sha256'], 'receiver_authority_invalid')
