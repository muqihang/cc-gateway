...
function executionRelative(row: RunLedgerRow, state: string): string {
  return `execution-records/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}-${state}.json`
}

function collectSealedArtifacts(root: string, ledger: CampaignLedger): readonly ArtifactEntry[] {
  const entries: ArtifactEntry[] = []
  const add = (relative: string) => { const artifact = optionalCanonical(root, relative); if (artifact) entries.push(artifact.entry) }
  for (const relative of ['control/campaign-input.json', 'control/operator-authority.json', 'prelaunch/run-ledger.json', 'prelaunch/static-anchor.json', 'prelaunch/result.json', 'launch-images.json', 'target-controls/result.json', 'execution-result.json', 'campaign-failure.json', 'guard-denied.tmp']) add(relative)
  for (const row of ledger.rows) {
    for (const state of ['started', 'spawned', 'terminal', 'not_executed']) add(executionRelative(row, state))
    add(`launch-authorities/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}.json`)
    add(`receiver-authorities/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}.json`)
    add(`guards/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}.json`)
    add(`runs/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}/guard-allowed.tmp`)
    add(`runs/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}/input-descriptor.json`)
    add(`cell-results/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}.json`)
    add(`receiver-results/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}.json`)
    for (let attempt = 0; attempt < row.response_program.maximum_attempts; attempt += 1) add(`observations/${String(row.sequence_index).padStart(3, '0')}-${row.run_id}-${String(attempt).padStart(2, '0')}.json`)
  }
...
function validateObservation(value: Record<string, unknown>, row: RunLedgerRow): Readonly<{ sha256: string; projection: Readonly<Record<string, unknown>> }> {
  ...
  if (response) assertExactKeys(response, ['status', 'ordered_header_classes', 'body_byte_length', 'body_sha256', 'sse_event_order', 'transport_terminal', 'timing_bucket'], 'observation_invalid')
  ...
  const expectedTimingBucket = action.delay_class === 'bounded_before_headers' ? 'at_or_after_boundary' : 'not_observed'
  if (!action || !response || response.status !== action.status || response.transport_terminal !== action.transport_terminal || response.timing_bucket !== expectedTimingBucket || response.body_byte_length !== expectedBody.length || response.body_sha256 !== sha256Bytes(expectedBody) || sha256Canonical(response.ordered_header_classes) !== sha256Canonical(action.ordered_headers) || sha256Canonical(response.sse_event_order) !== sha256Canonical(expectedEvents)) throw new Phase3BProductionError('observation_invalid', 'actual response bytes/status/headers/events/timing/terminal drifted from sealed program')
...
function expectedRouteOrdinal(row: RunLedgerRow): number {
...
function classifyRow(root: string, row: RunLedgerRow, terminal: ExecutionReceipt | null): Readonly<Record<string, unknown>> {
  ...
  const selectedRoute = expectedRouteOrdinal(row)
...
export function deriveCuration(evidenceRoot: string): Readonly<Record<string, unknown>> {
  ...
  const clockUnsigned = { schema_id: 'oracle-lab-p3b-curation-clock.v1', campaign_id: ledger.campaign_id, ledger_sha256: ledger.ledger_sha256, receipt_set_sha256: sha256Canonical(receipts), predecessor_receipt_sha256: lastReceipt?.receipt_sha256 ?? null, created_at_ms: issuedAt, created_monotonic_ns: issuedMonotonic }
  ...
  const artifactEntries = collectSealedArtifacts(root, ledger)
...
function forbiddenMaterial(value: unknown): boolean {
  ...
  return /(?:BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY|\bsk-[A-Za-z0-9_-]{8,}|\bBearer\s+[A-Za-z0-9._~+\/-]{4,}|"(?:raw_prompt|raw_body|credential|secret|token)"\s*:)/i.test(text)
}

export function runCloseout(evidenceRoot: string): Readonly<Record<string, unknown>> {
  const root = assertPrivateRuntimeRoot(evidenceRoot)
  createPrivateDirectory(root, CLOSURE_ROOT)
  const ledger = validateCampaignLedger(readCanonical(root, 'prelaunch/run-ledger.json', 16_777_216).value)
  const store = openExecutionStore(root, ledger)
  const receipts = readExecutionReceipts(store)
  ...
  const entries = [...collectSealedArtifacts(root, ledger), ...[`${CURATION_ROOT}/clock-attestation.json`, `${CURATION_ROOT}/contradictions.json`, `${CURATION_ROOT}/coverage.json`, `${CURATION_ROOT}/payload-manifest.json`, `${CURATION_ROOT}/result.json`, ...CONCLUSION_IDS.map((id) => CONCLUSION_PATHS[id])]].map((relative) => typeof relative === 'string' ? optionalCanonical(root, relative)?.entry : relative).filter((entry): entry is ArtifactEntry => Boolean(entry)).sort((left, right) => utf8Compare(left.relative_path, right.relative_path))
  ...
  const findings = entries.filter((entry) => forbiddenMaterial(readCanonical(root, entry.relative_path, 16_777_216).value)).map((entry) => ({ relative_path: entry.relative_path, finding: 'forbidden_material' }))
