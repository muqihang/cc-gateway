...
export function assertControllerLaunchPrerequisites(controller: ProductionController): void {
  ...
  for (const field of ['cc_repository', 'sub_repository', 'cross_review_artifact_path', 'cross_repo_review_path', 'platform_archive_path', 'source_tree_path', 'toolchain_path', 'schema_bundle_path', 'focused_suite_path', 'es7_typed_fixtures_path', 'es8_go_receipt_path', 'es8_ts_c1_agreement_path', 'es9_coverage_contract_path', 'predecessor_config_auth_path', 'predecessor_failure_stream_path', 'probe_source', 'probe_unsigned_source', 'original_recipe', 'probe_recipe']) if (typeof input[field] !== 'string') throw new Phase3BProductionError('sealed_path_invalid', `${field} is not sealed`)
  ...
  const committedReview = readCanonical(root, 'control/implementation-review.json', 1_048_576)
  if (committedReview.identity.sha256 !== approval.implementation_review_sha256 || sha256Canonical(committedReview.value) !== sha256Canonical(approval.implementation_review)) throw new Phase3BProductionError('sealed_authority_file_drift', 'sealed implementation review does not equal the approval Git blob')
  const exactFiles = [
...
export function deriveLaunchAuthority(input: Readonly<{
  ...
  if (input.receiver_authority.anchor_sha256 !== state.anchorSha256 || input.receiver_authority.ledger_sha256 !== state.ledger.ledger_sha256 || launchImage.selected_executable_class !== input.row.selected_executable_class || launchImage.record_sha256 !== selectedRecord || launchImage.source_tree_sha256 !== anchor.source_tree_sha256 || launchImage.toolchain_sha256 !== anchor.toolchain_sha256 || launchImage.reviewed_artifact_set_sha256 !== anchor.reviewed_artifact_set_sha256) throw new Phase3BProductionError('launch_authority_invalid', 'receiver anchor or selected image source/tree/toolchain/review drifted')
  ...
  const runRoot = path.join(state.runtimeRoot, 'runs', `${String(input.row.sequence_index).padStart(3, '0')}-${input.row.run_id}`)
  const exactGuardProfileSha256 = sha256Bytes(Buffer.from(buildSandboxProfile(state.runtimeRoot, runRoot, input.receiver_authority.routes.map((route) => route.port)), 'utf8'))
  const unsigned = {
    ...
    request_stimulus_sha256: input.row.request_stimulus_sha256,
    ...
    guard_profile_sha256: exactGuardProfileSha256,
