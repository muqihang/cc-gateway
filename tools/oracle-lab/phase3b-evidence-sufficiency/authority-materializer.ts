...
function runFocusedSuite(repository: string): Readonly<Record<string, unknown>> {
  const focused = ['tests/oracle-phase3b-production-executor-red.test.ts', 'tests/oracle-phase3b-production-core.test.ts', 'tests/oracle-phase3b-targeted-closure-red.test.ts', 'tests/oracle-phase3b-authority-materialization-red.test.ts', 'tests/oracle-phase3b-signer-entry-protocol-red.test.ts']
  const sources = ['authority-materializer-cli.ts', 'authority-materializer.ts', 'campaign-controller.ts', 'closeout.ts', 'controller.ts', 'core.ts', 'ephemeral-signer.ts', 'requirements-signer-session-cli.ts', 'security-reviewer-session-cli.ts', 'execution-store.ts', 'gates.ts', 'launch-authority.ts', 'launch-image.ts', 'ledger.ts', 'operator-decision.ts', 'receiver.ts', 'route-policy.ts', 'sandbox-policy.ts', 'scenario-input.ts', 'sealed-fs.ts', 'source-identity.ts', 'spawn-adapter.ts', 'trust.ts'].map((name) => `tools/oracle-lab/phase3b-evidence-sufficiency/${name}`)
  const commands = [
...
export function materializeAuthorityArtifacts(input: AuthorityMaterializerInput): Readonly<Record<string, unknown>> {
  ...
  const schemaFiles = ['authority-materializer-cli.ts', 'authority-materializer.ts', 'campaign-controller.ts', 'closeout.ts', 'ephemeral-signer.ts', 'requirements-signer-session-cli.ts', 'security-reviewer-session-cli.ts', 'execution-store.ts', 'github-web-flow.gpg', 'github-web-flow-keyring.gpg', 'github-web-flow-keyring.kbx', 'github-web-flow-gnupg/pubring.kbx', 'github-web-flow-gpgv', 'launch-authority.ts', 'launch-image.ts', 'ledger.ts', 'receiver.ts', 'trust.ts'].map((name) => {
