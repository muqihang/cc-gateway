...
import { buildCrossRepoRecord, encodeCrossRepoRecord, executeCrossRepoRecord, isSafeTaskId, validateCrossRepoRecord } from '../../oracle-contract/check-cross-repo.js'
...
function runFocusedSuite(repository: string): Readonly<Record<string, unknown>> {
  const focused = ['tests/oracle-phase3b-production-executor-red.test.ts', 'tests/oracle-phase3b-production-core.test.ts', 'tests/oracle-phase3b-targeted-closure-red.test.ts', 'tests/oracle-phase3b-authority-materialization-red.test.ts', 'tests/oracle-phase3b-signer-entry-protocol-red.test.ts', 'tests/oracle-phase3b-pre-epoch-admission-red.test.ts']
  const sources = ['authority-materializer-cli.ts', 'authority-materializer.ts', 'campaign-controller.ts', 'closeout.ts', 'controller.ts', 'core.ts', 'ephemeral-signer.ts', 'requirements-signer-session-cli.ts', 'security-reviewer-session-cli.ts', 'pre-epoch-admission-cli.ts', 'pre-epoch-admission.ts', 'execution-store.ts', 'gates.ts', 'launch-authority.ts', 'launch-image.ts', 'ledger.ts', 'operator-decision.ts', 'receiver.ts', 'route-policy.ts', 'sandbox-policy.ts', 'scenario-input.ts', 'sealed-fs.ts', 'source-identity.ts', 'spawn-adapter.ts', 'trust.ts'].map((name) => `tools/oracle-lab/phase3b-evidence-sufficiency/${name}`)
  const commands = [
