...
export function importSignedOperatorDecision(evidenceRoot: string, signedDecisionPath: string): Readonly<Record<string, unknown>> {
  ...
  verifyTrustedSignature(parsed, loadTrustedReviewerRegistry(input.cc_repository), 'requirements', 'decision_sha256', 'operator_decision_invalid')
...
export function evaluateGateB(evidenceRoot: string): Readonly<Record<string, unknown>> {
  ...
  const { input, authority } = loadSealedControl(root)
  verifyTrustedSignature(decision, loadTrustedReviewerRegistry(input.cc_repository), 'requirements', 'decision_sha256', 'operator_decision_invalid')
