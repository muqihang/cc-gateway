...
import { CROSS_REPO_AUTHORITY, REPOSITORY_AUTHORITY, TARGET_PROFILE } from './ledger.js'
...
function readRecipe(file: string, kind: Recipe['kind'], sourceSha256: string, sourceTreeSha256: string, toolchainSha256: string): { recipe: Recipe; identity: StableFileIdentity } {
  const root = path.dirname(file)
  const relative = path.basename(file)
  const { value, identity } = readCanonical(root, relative, 32_768)
  assertExactKeys(value, ['schema_id', 'kind', 'source_sha256', 'source_tree_sha256', 'toolchain_sha256', 'semantics', 'code_signature_identity_sha256', 'recipe_sha256'], 'launch_recipe_invalid')
  ...
  if (value.schema_id !== 'oracle-lab-p3b-launch-recipe.v1' || value.kind !== kind || value.source_sha256 !== sourceSha256 || value.source_tree_sha256 !== sourceTreeSha256 || value.toolchain_sha256 !== toolchainSha256 || sha256Canonical(value.semantics) !== sha256Canonical(expectedSemantics)) throw new Phase3BProductionError('launch_recipe_invalid', 'recipe source/tree/toolchain/probe semantics drifted')
...
function recordImage(input: { selectedClass: LaunchImageRecord['selected_executable_class']; imagePath: string; sourceIdentity: StableFileIdentity; sourceTreeSha256: string; toolchainSha256: string; recipe: Recipe; recipeIdentity: StableFileIdentity }): LaunchImageRecord {
...
export function createSealedLaunchImages(input: Readonly<{ runtime_root: string; original_source: string; probe_source: string; original_recipe: string; probe_recipe: string; source_tree_sha256: string; toolchain_sha256: string }>): Readonly<{ original: LaunchImageRecord; probe: LaunchImageRecord }> {
  ...
  const originalRecipe = readRecipe(input.original_recipe, 'original', originalSource.sha256, input.source_tree_sha256, input.toolchain_sha256)
  const probeRecipe = readRecipe(input.probe_recipe, 'probe', probeSource.sha256, input.source_tree_sha256, input.toolchain_sha256)
  ...
  const result = deepFreeze({
    original: recordImage({ selectedClass: 'original_image', imagePath: originalPath, sourceIdentity: originalSource, sourceTreeSha256: input.source_tree_sha256, toolchainSha256: input.toolchain_sha256, recipe: originalRecipe.recipe, recipeIdentity: originalRecipe.identity }),
    probe: recordImage({ selectedClass: 'probe_image', imagePath: probePath, sourceIdentity: probeSource, sourceTreeSha256: input.source_tree_sha256, toolchainSha256: input.toolchain_sha256, recipe: probeRecipe.recipe, recipeIdentity: probeRecipe.identity }),
  })
...
export function loadLaunchImageRecord(value: unknown): LaunchImageRecord {
  assertExactKeys(value, ['schema_id', 'selected_executable_class', 'image_identity', 'source_identity', 'source_tree_sha256', 'toolchain_sha256', 'recipe_sha256', 'recipe_identity', 'recipe_identity_sha256', 'probe_semantics_sha256', 'code_signature_identity_sha256', 'record_sha256'], 'launch_image_invalid')
  ...
  for (const field of ['source_tree_sha256', 'toolchain_sha256', 'recipe_sha256', 'recipe_identity_sha256', 'probe_semantics_sha256'] as const) assertSha256(value[field], 'launch_image_invalid', field)
  const record = deepFreeze(value as LaunchImageRecord)
  images.add(record)
  return verifyLaunchImage(record)
}

export function buildStaticAnchor(input: Readonly<{ platform_archive_sha256: string; source_tree_sha256: string; toolchain_sha256: string; images: Readonly<{ original: LaunchImageRecord; probe: LaunchImageRecord }>; receiver_source_sha256: string; receiver_executable_identity_sha256: string; receiver_schema_sha256: string; controller_source_sha256: string; controller_executable_sha256: string; schema_bundle_sha256: string }>): StaticAnchor {
  for (const field of ['platform_archive_sha256', 'source_tree_sha256', 'toolchain_sha256', 'receiver_source_sha256', 'receiver_executable_identity_sha256', 'receiver_schema_sha256', 'controller_source_sha256', 'controller_executable_sha256', 'schema_bundle_sha256'] as const) assertSha256(input[field], 'static_anchor_invalid', field)
