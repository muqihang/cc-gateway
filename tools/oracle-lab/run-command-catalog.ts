...
import { commandRecordDigest, commandSetDigest, type CommandResultRecord, type CommandResultSet } from './merge-command-results.js'
...
function worktreeDelta(root: string): string[] {
  return gitOutput(root, ['status', '--porcelain=v1', '--untracked-files=all']).split('\n').filter(Boolean).map((line) => line.slice(3).replace(/^"|"$/g, '')).filter((entry) => {
    if (entry !== 'node_modules') return true
    try { return !lstatSync(path.join(root, entry)).isSymbolicLink() } catch { return true }
  }).sort()
}
...
function binding(entry: CommandCatalogEntry, roots: RuntimeRoots): { manifestPath: string; manifest: Record<string, unknown>; manifestDigest: string; root: string; commit: string } {
  ...
  const delta = worktreeDelta(root)
  const allowed = [...entry.allowed_worktree_delta].sort()
  if (delta.some((entry) => !allowed.includes(entry))) throw Object.assign(new Error(`${entry.id} worktree delta is ${JSON.stringify(delta)}`), { code: 'worktree_delta_mismatch' })
  return { manifestPath, manifest, manifestDigest: digestFile(manifestPath), root, commit }
}
...
export async function runCommandCatalog(catalogPath: string, group: 'phase0-green' | 'phase0-red', rootsInput: Partial<RuntimeRoots>, generatedAt = new Date().toISOString()): Promise<CommandResultSet> {
  ...
  for (const entry of selected) {
    ...
    const status = entry.expected_exit === 0 ? (observed.exitCode === 0 ? 'pass' : 'unexpected_fail') : (observed.exitCode !== 0 ? 'expected_fail' : 'unexpected_pass')
    const outputDigest = sha256(observed.output)
    const unsigned = { command_id: entry.id, repository: entry.repository, repository_commit: bound.commit, manifest_digest: bound.manifestDigest, environment_digest: environmentDigest, exit_code: observed.exitCode, expected_exit: entry.expected_exit, status, output_digest: outputDigest, ...(entry.output_policy === 'redacted_excerpt' ? { output_excerpt: redact(observed.output.toString('utf8')) } : {}) }
