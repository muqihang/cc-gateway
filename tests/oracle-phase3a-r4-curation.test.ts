...
assert.throws(() => assertTierAPairedSchedule({ repetitions: 13, runs: Array.from({ length: 26 }, (_, index) => ({ arm: index % 2 === 0 ? 'control' : 'treatment', repetition: Math.floor(index / 2), run_id: `run-${index}` })) }), (error: any) => error.code === 'r4_terminal_source_invalid')
