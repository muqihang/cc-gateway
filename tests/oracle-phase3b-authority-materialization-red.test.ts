...
test('authority RED: requirements signer session retains one key for launch authority and later Gate B decision', () => {
  ...
  assert.throws(() => verifyTrustedSignature({ ...decision, issued_at_ms: 13 } as Record<string, unknown>, registry, 'requirements', 'decision_sha256', 'operator_decision_invalid'), (error: Error & { code?: string }) => error.code === 'operator_decision_invalid')
  const tamperedUnsigned = { ...decision, issued_at_ms: 13 }
  delete (tamperedUnsigned as { decision_sha256?: unknown }).decision_sha256
  assert.throws(() => verifyTrustedSignature({ ...tamperedUnsigned, decision_sha256: sha256Canonical(tamperedUnsigned) } as Record<string, unknown>, registry, 'requirements', 'decision_sha256', 'operator_decision_invalid'), (error: Error & { code?: string }) => error.code === 'operator_decision_invalid')
