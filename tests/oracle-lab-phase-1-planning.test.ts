...
const expectedGateSchemaDigests = {
  execution_context: 'sha256:679484ecae101b566db110e2a0eece952289e838ec20911d6563ee2cc139d9d2',
  plan_review: 'sha256:82ff0c9eea348c4663440ce84d3bd2fe8c91a2142d679cfc58e7201964a1f43e',
} as const
const recoveryPlanDigest = 'sha256:8c1d2a9ec1532f30d241a41d00d24998d37ac1f0babb45cd5b945a7c2405fcf2'
const recoveryReviewedCommit = '8981ee2607de6df2310d198e2a0669190949b718'
...
test('Phase 1 mid-execution plan repair restarts canonical initial authority instead of mutating a successor chain', async () => {
  ...
  assert.match(plan, /materializes only that immutable snapshot in an exclusive command-scoped `\/tmp` tool root.*runs `validatePhase1AuthorityRestartSource`.*before any replacement-root mutation/)
  assert.match(plan, /materializes the complete launcher\/bootstrap\/tool\/schema\/secure-runtime\/package\/lock snapshot with exclusive creation inside a mode-0700 command-scoped `\/tmp\/oracle-phase1-authority-dependencies\.\*` root/)
  assert.match(plan, /`validate-source` returns without installing or mutating any replacement dependency/)
  assert.match(plan, /No authority command runs `npm ci` in either replacement root/)
  assert.match(plan, /No post-verification replacement of a live tool, schema, secure runtime, package manifest, or lock can affect command dispatch or dependency selection/i)
  assert.match(plan, /Regression tests atomically replace the live tool and manifest\/lock after runtime verification/)
