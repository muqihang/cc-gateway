...
assert.throws(() => buildR4TerminalManifest({ ...input, tier_a_rerun: { ...tier_a_rerun, pair_outcomes: tier_a_rerun.pair_outcomes.map((outcome: any, index) => index === 0 ? { ...outcome, capability_evidence: { ...outcome.capability_evidence, result_count: 26, terminal_result_count: 26, process_sampled_result_count: 26, safe_diagnostic_result_count: 26 } } : outcome) } } as any), /terminal rerun/)
