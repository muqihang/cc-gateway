...
- Delivery state: roadmap only; no implementation phase is complete
...
This amendment is authoritative for roadmap navigation. The dependency DAG, rather than serial
wording elsewhere in this document, controls start conditions:
...
exit gates plus all applicable prior gates. Replace any “previous phase” wording with “all declared
predecessor gates”; runtime integration and promotion remain blocked until the relevant predecessor
gates pass.
...
### Evidence-to-Decision and Artifact Exit Gates

Phase 3 must emit an evidence-to-decision matrix whose every row binds immutable evidence digests,
exact scope, independent wire/semantic/state/failure compatibility verdicts, explicit negative
capabilities, an exact target change (`no_change|strip|disable|add|replace`), named owner and
reviewer, maximum authority, blocking contradictions/expiry, rollback reference, and promotion
gate. Unknown, contradictory, expired, parser-disagreeing, or unexplained-transport rows are
disabled and cannot become acceptable in Phase 4.
