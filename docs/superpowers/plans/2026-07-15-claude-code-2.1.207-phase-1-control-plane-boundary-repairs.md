...
1. Preserve the in-flight delta without modifying the operator-owned sibling. If the task began under a then-valid context, it may finish only to one clean **quarantine checkpoint** commit whose parent is the last clean reviewed task head. The checkpoint makes no PASS, approval, capture, or completion claim; record its exact commit, parent, changed-path list, patch digest, and implementation-tree digest in the external controller decision package.
...
5. After the initial post-commit gate passes, create `docs/superpowers/evidence/phase-1/phase-1-authority-restart-0001.json` with `buildPhase1AuthorityRestart`, validate it against `docs/superpowers/schemas/oracle-lab-phase-1-authority-restart.schema.json`, and replay only the enumerated reviewed Task 1-6 implementation commits and the Task 7 quarantine checkpoint. The replay must not cherry-pick the superseded plan review or execution-context artifacts. Preserve commit order and repository ownership. Reject merge commits, empty commits, extra paths, or a source commit not in the enumerated set.
6. `validatePhase1AuthorityRestart` independently uses `runReviewedGit` to prove patch-id and implementation-tree equivalence: stable patch-id, exact parent order, and exact changed-path/mode set for every source-to-replacement mapping. It proves the projected tracked-tree comparison excludes exactly the reviewed authority-repair path set and canonical old/new review-context paths; all nonexcluded projected path, mode, object-type, and object-ID tuples remain byte-identical. A prose decision package, aggregate diff summary, user-supplied digest, or conflict-resolved cherry-pick is never equivalence evidence. Any mismatch stops replay and requires review.
7. Commit the validated restart artifact as a one-path child of the final replay head before Task 7 broad capture. The restart artifact is committed as a one-path child before Task 7 broad capture, and its post-commit validator proves the sole parent, sole added path, committed-byte digest, replacement branch names/heads, latest sequence-zero context digest, and both clean worktrees. Refresh CodeGraph in both replacement worktrees and rerun the task-scoped tests plus planning, P0.1, three isolated full-suite passes, and build. Only after these gates pass may Task 7 broad validation resume under the replacement initial context. Keep superseded worktrees read-only until replay and independent closing review are complete; removal remains a separately approved cleanup.

For authority-repair instance `0001`, the superseded clean heads are CC `49e4639c6f36dc51779c14813acd6e277315b969` and Sub2API `e2af1be5176854958a3d7b63a029174ffc5792a8`; the contract clone remains `main@b0b77933716487da5fca00329443f88ce9a1c3db`. CC replay excludes authorization commit `dd5ea716bc84e391daec333ecf03f41643612dde` and permits only Task 6 commits `2a1553a4d16ccfdcd186ae78d99deeecbd7dfb4c` and `49e4639c6f36dc51779c14813acd6e277315b969`, followed by the recorded one-parent Task 7 quarantine checkpoint. Sub2API replay permits, in order, exactly `b095307407b7b0bf08a7fc629a5d83dea86c26ab`, `fadcbd18c0d49bf3562e568fd6b8282c4417a12c`, `6f1754396b572abf929cb80ea4602306b89fcf9c`, `7aba29c7387b82e37187d11c80ced09cef86b47f`, `4e55cb8b0442c3a0f1734615efaf38967f2fe1aa`, `3b09da2574e07a72e0cac34a28aeb5d4604f4759`, `fe7753fa5b0b046eea42427e29aab3af467d5312`, `540d58cb820811e7beaca26e678f499c8cc66351`, `d25ecc1ddf1cf1e058c903c26915cf11c9c97025`, and `e2af1be5176854958a3d7b63a029174ffc5792a8`. These source SHAs identify replay inputs only; replacement commits receive new SHAs and must satisfy the equivalence and test gates above.

The restart schema is closed with `additionalProperties: false` at every level. It freezes `schema_version: 1`, `repair_id: authority-repair-0001`, the superseded and replacement remote origins/branches/base heads, the repaired plan commit/digest, plan-review path/digest, sequence-zero execution-context path/digest/artifact commit, quarantine checkpoint/parent, ordered repository-specific replay mappings, projected-tree policy, replay heads, and artifact commit. Every mapping contains the source and replacement commit/sole parent, stable patch-id, and UTF-8 byte-sorted exact changed-path/mode/status tuples; rename entries bind both old and new paths. The projected-tree policy excludes exactly these authority-repair paths: `docs/superpowers/plans/2026-07-15-claude-code-2.1.207-phase-1-control-plane-boundary-repairs.md`, `tests/oracle-lab-governance-amendment-evidence.test.ts`, `tests/oracle-lab-ignored-path-inventory.test.ts`, `tests/oracle-lab-phase-1-planning.test.ts`, `tests/suite-process-runner.ts`, and `tests/suite-process-runner.test.ts`, plus exact canonical historical authority paths `docs/superpowers/evidence/phase-1/phase-1-context.json`, `docs/superpowers/evidence/phase-1/phase-1-entry-baseline.json`, `docs/superpowers/evidence/phase-1/phase-1-plan-review.json`, and `docs/superpowers/evidence/phase-1/phase-1-execution-context.json`. Prefix, glob, regex, caller-supplied exclusion, successor-context wildcard, or any additional path is forbidden.
...
**Merged authority prerequisite:** `package.json` launches the full suite as `node --import tsx tests/run-all.ts`, without the `tsx` CLI IPC server. `tests/run-all.ts` is only an orchestrator. It invokes the reviewed `tests/suite-process-runner.ts`, which runs `tests/run-p0-1.ts` and then `tests/run-all.ts --exclude-oracle-p0-1` serially in two distinct child processes. `buildClosedFullSuiteEnvironment` constructs each default child environment from a fixed allowlist rather than copying `process.env`: the inventory-bound repository-local `node_modules/.bin`, fixed reviewed system tool paths, `/tmp` home/temp, C locale, UTC, user npm config `/dev/null`, global npm config `/nonexistent/oracle-lab-empty-global-npmrc`, offline Go/npm settings, an OS-account-derived reviewed Go module-cache realpath, `GOFLAGS=-mod=readonly`, a parent-PID-specific `/tmp` Go build cache, loopback-denial proxies, and optionally the one dedicated `SUB2API_FORMAL_POOL_CONTRACT_PATH`. It omits `SUB2API_ROOT`, every `ORACLE_*`, `PHASE1_*`, `GIT_*`, and undeclared npm variable, and fails `unsafe_full_suite_environment` before spawn when Node loader, custom CA, tsx config, or dynamic-library startup injection is inherited. PID, environment-leak, child-order, abnormal-exit, nonzero-exit, exact allowlist, dependency-cache-unavailable, and startup-injection regression tests live in `tests/suite-process-runner.test.ts` and `tests/run-all-process-isolation.test.ts`. No top-level P0.1 assertion module shares a Node process with asynchronous `node:test` registration. Repeated P0.1 clone fixtures derive each temporary branch suffix from the already-created unique temporary parent and never force, delete, or reuse a prior fixture ref. The ignored-state build regression is self-contained in a fresh worktree: it proves the build output is initially absent, performs one seed build before its before snapshot, then proves the immediate repeat build is byte/mode stable. Mandatory Preflight retains `npm test` as GREEN only after three consecutive `env -i` isolated `npm test` runs pass on one unchanged clean HEAD.
...
- Create: `docs/superpowers/schemas/oracle-lab-phase-1-authority-restart.schema.json`
...
export type Phase1ExternalDependencyBinding = {
  algorithm: 'phase1_go_module_content_v1'
...
The external Go dependency surface is also closed. Before and after every command that can invoke Go, derive `Phase1ExternalDependencyBinding` from the reviewed Go executable digest, tracked `go.mod`/`go.sum` bytes, `go list -mod=readonly -m -json all` under `GOENV=off`, `GOPROXY=off`, `GOSUMDB=off`, `GOTOOLCHAIN=local`, and the complete content/mode/type digest of every selected module directory inside the OS-account-derived canonical `GOMODCACHE` realpath. Reject symlinks, module directories outside that root, duplicate module path/version pairs, replacement directives outside a tested repository, missing sums, additions, deletions, content or mode drift, and any before/after digest change as `external_dependency_drift`. Persist only counts and digests, never absolute cache paths. Go compile outputs use the invocation parent's PID-specific `/tmp/oracle-lab-phase1-go-build-<pid>` `GOCACHE`; that cache is not reused by a later full-suite invocation and is never an input authority. Node dependencies remain inside the already-bound `node_modules` ignored inventory. Mutation tests change each external module identity/content/mode/type, Go binary, go.mod/go.sum byte, replacement target, and build-cache reuse token independently through capture, results, integration entry, handoff, receipt, and final-remote validation.
