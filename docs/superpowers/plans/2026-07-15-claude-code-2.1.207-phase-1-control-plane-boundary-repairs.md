...
The restart schema is closed with `additionalProperties: false` at every level. It freezes `schema_version: 1`, `repair_id: authority-repair-0001`, the superseded and replacement remote origins/branches/base heads, the repaired plan commit/digest, plan-review path/digest, sequence-zero execution-context path/digest/artifact commit, quarantine checkpoint/parent, ordered repository-specific replay mappings, projected-tree policy, replay heads, and artifact commit. Every mapping contains the source and replacement commit/sole parent, stable patch-id, and UTF-8 byte-sorted exact changed-path/mode/status tuples; rename entries bind both old and new paths. The projected-tree policy excludes exactly these authority-repair paths: `docs/superpowers/plans/2026-07-15-claude-code-2.1.207-phase-1-control-plane-boundary-repairs.md`, `tests/oracle-lab-ignored-path-inventory.test.ts`, `tests/oracle-lab-phase-1-planning.test.ts`, `tests/suite-process-runner.ts`, and `tests/suite-process-runner.test.ts`, plus exact canonical historical authority paths `docs/superpowers/evidence/phase-1/phase-1-context.json`, `docs/superpowers/evidence/phase-1/phase-1-entry-baseline.json`, `docs/superpowers/evidence/phase-1/phase-1-plan-review.json`, and `docs/superpowers/evidence/phase-1/phase-1-execution-context.json`. Prefix, glob, regex, caller-supplied exclusion, successor-context wildcard, or any additional path is forbidden.
