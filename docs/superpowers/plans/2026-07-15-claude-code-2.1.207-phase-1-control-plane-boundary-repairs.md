...
3. Create fresh replacement implementation worktrees. First rename the superseded implementation branches to immutable `*-pre-authority-repair-0001` archival names, then recreate the exact implementation branch names required by this plan. After branch recreation, the replacement CC branch starts at the newly reviewed merged plan commit, and the replacement Sub2API branch starts at freshly fetched `muqihang/main`. Do not reset, rebase, or mutate either superseded worktree.
...
5. The repaired main already contains reviewed `docs/superpowers/schemas/oracle-lab-phase-1-authority-restart.schema.json`, `tools/oracle-lab/phase-1-authority-restart.ts`, and `tests/oracle-lab-phase-1-authority-restart.test.ts`; they are Mandatory Preflight inputs, not outputs of the checkpoint being replayed. After the initial post-commit context gate passes, run `validatePhase1AuthorityRestartSource` against the hard-coded source histories and exact checkpoint before changing either replacement branch. Then replay only the enumerated Task 1-6 commits and pinned Task 7 checkpoint. Do not replay superseded review/context artifacts. Preserve commit order and repository ownership; reject merge/empty commits, conflict resolution, extra paths, or any source object not hard-coded in the reviewed tool.
6. `validatePhase1AuthorityRestart` independently uses `runReviewedGit` to prove patch-id and implementation-tree equivalence: stable patch-id, exact parent order, and exact changed-path/mode set for every source-to-replacement mapping. Before mapping acceptance it proves the CC replay base is the unique one-parent initial-authority child of repaired remote main whose exact delta adds only the bound plan-review and sequence-zero context bytes, and proves the Sub2API replay base equals frozen remote main. It proves the projected tracked-tree comparison excludes exactly the reviewed authority-repair path set and canonical old/new review-context paths; all nonexcluded projected path, mode, object-type, and object-ID tuples remain byte-identical. A caller-derived first parent, intervening excluded-path commit, prose decision package, aggregate diff summary, user-supplied digest, or conflict-resolved cherry-pick is never equivalence evidence. Any mismatch stops replay and requires review.
7. After replay, build and pre-commit validate `docs/superpowers/evidence/phase-1/phase-1-authority-restart-0001.json`, then commit it as a one-path child of the final replay head. The JSON deliberately omits its own digest and commit. Post-commit validation derives `HEAD`, proves its sole parent is the artifact's bound replay head, proves the sole delta adds the canonical restart path with exact committed bytes, and revalidates replacement branch names/heads, latest sequence-zero context digest, both clean worktrees, every mapping, and both projected trees. The derived commit is runtime output only and is not written back into the artifact.
8. The replayed checkpoint is explicitly incomplete Task 7 work. Resume Task 7 only after the restart post-commit gate passes, implement the repaired contract-environment, external-dependency, cache, schema/reference, and mutation requirements in the replayed Task 7 files, then run task-scoped tests, planning, P0.1, three isolated full-suite passes, build, and independent review. Commit this nonempty repair-specific continuation as the final Task 7 implementation commit, then proceed to Task 8. Keep superseded worktrees read-only until replay and closing review complete; removal remains separately approved cleanup.
...
The external dependency surface is also closed. Preparation runs exact `npm ci --offline --ignore-scripts` in each reviewed Node dependency root and exact `go mod verify` in each Go module root before the baseline snapshot; a preexisting `node_modules` tree, failed install/verify, lock drift, lifecycle script, network fallback, or unreviewed binary fails before capture. Derive one `Phase1ExternalDependencyBinding` per tested repository from reviewed Node/npm/Go executable digests; tracked package/lock and go.mod/go.sum bytes; the complete path/mode/type/content digest of each resulting `node_modules` tree; `go list -mod=readonly -m -json all` under `GOENV=off`, `GOFLAGS=-mod=readonly`, `GOPROXY=off`, `GOSUMDB=off`, `GOTOOLCHAIN=local`; the successful `go mod verify` digest; and the complete content/mode/type digest of every selected module directory inside the canonical `GOMODCACHE` realpath. This authenticates initial bytes as well as detecting later drift. Reject symlinks outside the bound dependency root, module directories outside `GOMODCACHE`, duplicate module path/version pairs, replacement directives outside a tested repository, missing sums, additions, deletions, content/mode drift, or any before/after digest change as `external_dependency_drift`. Persist only repository-relative roots, counts, and digests, never absolute cache paths.

`Phase1CaptureRootEnvelope.external_dependencies` binds the exact initial `Phase1ExternalDependencySet`; every `Phase1Result.external_dependency_transition` binds before/after sets plus token `command_scoped_empty_mkdtemp_v1`; and `Phase1Results.external_dependency_chain` closes all 17 contiguous transitions. Feature review, integration entry, handoff, receipt, and final-remote artifacts each embed `Phase1ExternalDependencyEvidenceReference`, reload the referenced results, recompute both repositories, and reject absent/extra/rehashed fields. Their schemas use closed `const`/`prefixItems` branches and `additionalProperties: false`. `captureAndRunPhase1`, `validatePhase1ResultsValue`, `buildPhase1IntegrationEntry`, `validatePhase1IntegrationEntryValue`, `buildPhase1Handoff`, `validatePhase1HandoffValue`, `buildPhase1IntegrationReceipt`, both receipt validation modes, and `verifyPhase1FinalRemote` repeat this validation. Mutation tests change each Node module identity/content/mode/type, package/lock byte, external Go module identity/content/mode/type, Go binary, go.mod/go.sum byte, replacement target, preparation result, reference, chain adjacency, and build-cache token independently at every downstream stage; consistently rehashing a forged artifact still fails `external_dependency_drift`.
...
In addition to the ignored-state checks above, preparation must complete exact offline npm installation and Go module verification before the baseline snapshot. Capture the initial `Phase1ExternalDependencySet`; recompute it before and after all 17 commands; attach one `Phase1ExternalDependencyTransition` to every result; and close the chain in `Phase1ExternalDependencyChainBinding`. Every downstream builder and validator reloads the referenced results and embeds/revalidates `Phase1ExternalDependencyEvidenceReference`. The actual random `GOCACHE` path is never persisted or hashed; environment evidence substitutes the constant `command_scoped_empty_mkdtemp_v1` only after the exclusive cache checks pass.
...
Authority restart is deliberately not a subcommand of the replayed Task 7 adapter. It is provided by the already reviewed `tools/oracle-lab/oracle-phase1-authority-restart` hermetic launcher and `tools/oracle-lab/phase-1-authority-restart.ts` on repaired main, with exact commands `validate-runtime`, `validate-source`, `build`, `validate-pre-commit`, and `validate-post-commit`. `validate-runtime` is a no-side-effect positive startup probe that accepts only `--cc-replacement-root`; it must pass before `validate-source`. The launcher rejects Node/tsx, dynamic-library, and Git startup injection before Node starts; selects reviewed absolute Node/Git/npm executables; verifies the working launcher, tool, schema, secure runtime, and lock bytes against fetched repaired main; and resolves the canonical OS account from the account database rather than inherited `HOME`. The canonical account npm cache is an untrusted content seed only: it must be a real account-owned directory with no group/world write, is copied with APFS copy-on-write semantics into an exclusive mode-0700 command-scoped cache, and never supplies authority or persisted evidence. Under `env -i`, exact `npm ci --offline --ignore-scripts --cache <command-scoped-cache>` authenticates package bytes against the reviewed lockfile integrity before Node loads tsx/Ajv; writes to the command cache cannot modify the account cache. Inherited `HOME`, `npm_config_cache`, user config, global config, and network fallback cannot select dependency bytes. The actual cache paths are never persisted; the CLI receives only token `command_scoped_lockfile_verified_v1`, repeats runtime-byte verification, and requires its module root to equal the tested CC replacement root. Direct Node/tsx invocation is forbidden. Their closed parser accepts only explicit CC/Sub2API source and replacement roots, reviewed plan/review/context paths, canonical restart output path, and gate mode; source SHAs, checkpoint, tuples, branch names, exclusion lists, and artifact path are compiled constants rather than CLI or environment inputs.
