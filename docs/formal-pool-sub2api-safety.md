...
## Gateway compromise boundary

Formal-pool configuration must explicitly set `shared_pool.gateway_compromise_boundary`; there is no implicit default. Phase 0 selects `protected_gateway` for local-capture and preflight work. In that mode a production or real-canary startup fails closed with `protected_gateway_authority_unavailable` until the Phase 4 policy broker is locally verified and can issue independently authorized, short-lived request capabilities. This deferred production capability does not block Phase 0 governance exit while production and real-canary remain disabled; the Phase 4 policy broker remains a mandatory later gate.

`trusted_gateway` is the only Phase 0 production-compatible alternative. It deliberately places Gateway-process compromise out of scope: a compromised Gateway is outside the sidecar prevention boundary, and sidecar authentication cannot prevent malicious-but-valid requests generated by that Gateway. Deployments choosing `trusted_gateway` must preserve protection against external, loopback-peer, and same-container unprivileged attackers, state the limitation in release claims and telemetry, and prioritize Gateway isolation, secret rotation, manifest revocation, and forensic preservation during incident response.

The normalized boundary value is safe to copy into a run manifest and generated evidence. Raw keys, capabilities, credentials, request bodies, and broker decision inputs are not.
...
| Gateway compromise boundary | Normalized `protected_gateway` or `trusted_gateway` choice and gate result | Broker keys, capabilities, raw decision inputs, or claims that exceed the selected boundary |
...
- `protected_gateway` production and real-canary authority remain deferred to the mandatory Phase 4 policy broker gate; this does not block Phase 0 governance exit with those capabilities disabled.
